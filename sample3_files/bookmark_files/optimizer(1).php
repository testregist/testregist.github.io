...
var link = {
    ...
    {
        if (this.checkNavigator() == false) return false;
        if (shortcutJson['isUse'] == 'N') {
            alert(__('바로가기를 생성할 수 없습니다.'));
            return false;
        }


        try {
            $('#shortcut')[0].addIcon(shortcutJson['host'] + '/exec/front/Link/Redirect/?mediaType=shortcut', shortcutJson['mallName'], shortcutJson['icon']);
            this.setAward('Shortcut');
        } catch(e) {
            alert(__('엑티브 엑스를 설치해 주세요.'));
        }
    },
    ...
    {
        if (this.checkNavigator() == false) {
            window.open('','_self').close();
            return false;
        }
        if (bookmarkJson['isUse'] == 'N') {
            alert(__('즐겨찾기를 추가할 수 없습니다.'));
            return false;
        }
        
        try {
            // ECHOSTING-103014 active 제거
            window.external.AddFavorite(bookmarkJson['host'] + "/exec/front/Link/Redirect/?mediaType=bookmark", bookmarkJson['mallName']);
            this.setAward('Bookmark');
        } catch(e) {
            alert(__('처리오류! 관리자에게 문의하세요.'));
        }
    },
    ...
    checkNavigator : function()
    {
        if (navigator.appName != 'Microsoft Internet Explorer') {
            //ECHOSTING-72580 ie 11 대응합니다.
            if (navigator.appName == 'Netscape' && new RegExp('Trident/.*rv:([0-9]{1,}[\.0-9]{0,})').exec(navigator.userAgent) == null) {
               alert(__('이용하시는 브라우저에서는 기능이 지원되지 않습니다.') + '\n' + __('Ctrl+D키를 누르면 즐겨찾기에 추가하실 수 있습니다.'));
               return false;
            } else {
               return true; 
            }
    ...
    /**
     * ajax 호출 하여 보상
     * @param string sType (Shortcut | Bookmark)
     */
    setAward : function(sType)
    {
        if (this.bIsSubmit == true) return false;

        this.bIsSubmit = true;


        $.ajax({
            url: '/exec/front/Link/Award' + sType,
            cache: false,
            type: 'GET',
            dataType: 'json',
            timeout: 1000,
            success: function(response) {
                ...
                if (response.passed == false) alert(__('설치 되었습니다.'));

                window.close();//설치 완료 후 닫기
            }
        });
